...
class Point(object):
    ...
    def __repr__(self):
        """Returns string that represents point object"""
        return "Point(" + str(self.x) + ", " + str(self.y) + ")"

    def clone(self):
        """Creates and returns new Point with same x and y as self"""
        return Point(self.x, self.y)

    def move_to(self, x, y):
        """Moves point to new Point"""
    ...
    def move_by(self, dx, dy):
        """Moves Point by a certain dx and dy"""
        distance1 = math.sqrt(dx ** 2 + dy ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
        self.distance = self.distance + distance1

    def get_number_of_moves_made(self):
        """Returns the number of times the move_to and move_by functions have been called"""
        return self.moves

    def get_distance_from(self, point):
        """Finds the distance from self to another point"""
        return math.sqrt((point.x - self.x) ** 2 + (point.y - self.y) ** 2)

    def get_distance_from_start(self):
        """Returns the distance from self's position from the position from which it was constructed"""
        return math.sqrt((self.x - self.startx) ** 2 + (self.y - self.starty) ** 2)

    def get_distance_traveled(self):
        """Returns the total distance self traveled from being constructed"""
